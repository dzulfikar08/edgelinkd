...
pub type ChatStream =
    Pin<Box<dyn Stream<Item = Result<ChatChunk, AiError>> + Send>>;
...
pub trait AiProvider: Send + Sync {
    ...
    async fn suggest_nodes(
        ...
        context: &str,
        current_flow: &str,
    ) -> Result<Vec<NodeSuggestion>, AiError>;
...
impl AiProvider for OpenAiCompatibleProvider {
    ...
    async fn suggest_nodes(
        ...
        let suggestions: Vec<NodeSuggestion> =
            serde_json::from_str(&resp.message.content).unwrap_or_else(|e| {
...
impl OpenAiCompatibleProvider {
    ...
    ) -> impl Stream<Item = Result<ChatChunk, AiError>> {
        ...
        byte_stream.scan(
            ...
            |buffer, chunk_result| {
                let chunk = match chunk_result {
                    Ok(c) => c,
                    Err(e) => {
                        return futures_util::future::ready(Some(vec![Err(AiError::Streaming(
                            e.to_string(),
                        ))]));
                    }
                };
...
impl AnthropicProvider {
    pub fn new(config: AnthropicConfig) -> Self {
        ...
        headers.insert(
            "anthropic-version",
            config.api_version.parse().unwrap(),
        );
        if !config.api_key.is_empty() {
            headers.insert(
                "x-api-key",
    ...
    async fn chat(
        ...
        Ok(ChatResponse {
            ...
            usage: Some(TokenUsage {
                ...
                total_tokens: chat_resp
                    .usage
                    .input_tokens
                    .zip(chat_resp.usage.output_tokens)
                    .map(|(i, o)| i + o),
            }),
        })
    }
...
impl AiProvider for LocalWasmProvider {
    ...
    async fn chat(
        ...
        Err(AiError::ProviderUnavailable(
            "Local WASM inference is not yet implemented".to_string(),
        ))
    }
    ...
    match config.default_provider.as_str() {
        "openai-compatible" => {
            if !config.providers.openai_compatible.is_configured() {
                return Err(AiError::NotConfigured(
                    "openai-compatible provider is not configured".to_string(),
                ));
            }
            Ok(Box::new(OpenAiCompatibleProvider::new(
                config.providers.openai_compatible.clone(),
            )))
        }
        "anthropic" => {
        ...
        "local-wasm" => {
            if !config.providers.local_wasm.is_configured() {
                return Err(AiError::NotConfigured(
                    "local-wasm provider is not configured".to_string(),
                ));
            }
            Ok(Box::new(LocalWasmProvider::new(
                config.providers.local_wasm.clone(),
            )))
        }
