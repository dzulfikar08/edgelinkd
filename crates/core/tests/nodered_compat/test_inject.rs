...
use super::harness::{assert_msg_bool, assert_msg_eq, assert_msg_has, assert_msg_num, assert_msg_str, TestHarness};
...
async fn inject_string_payload() {
    let flow = FlowBuilder::new()
        .inject_once("1", "hello world", "str", json!([["99"]]))
        .test_sink("99")
        .to_json();
...
async fn inject_generates_msgid() {
    let flow = FlowBuilder::new()
        .inject_once("1", "test", "str", json!([["99"]]))
        .test_sink("99")
        .to_json();
